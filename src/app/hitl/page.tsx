...
const HumanInTheLoop: React.FC = () => {
  return (
    <CopilotKit runtimeUrl="/api/copilotkit" showDevConsole={false} agent="human_in_the_loop">
      <Chat />
    </CopilotKit>
  );
};
    ...
    <StepContainer theme={theme}>
      ...
      <div className="space-y-3 mb-6">
        {localSteps.map((step, index) => (
          <StepItem key={index} step={step} theme={theme} onToggle={() => handleStepToggle(index)} />
        ))}
      </div>
...
const StepsFeedback = ({ args, respond, status }: StepsFeedbackProps) => {
  ...
  if (status === ToolCallStatus.Executing && args.steps !== seededSteps && Array.isArray(args.steps) && args.steps.length > 0) {
  ...
  return (
    <StepContainer theme={theme}>
      ...
      <DecorativeElements theme={theme} variant={accepted === true ? "success" : accepted === false ? "danger" : "default"} />
    </StepContainer>
  );
};
